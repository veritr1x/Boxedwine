...
#ifdef __TEST
static bool g_wasmJitTestFailInvalidationPreparation = false;

void wasmJitTestSetFailInvalidationPreparation(bool fail) {
    g_wasmJitTestFailInvalidationPreparation = fail;
}
#endif
#include <vector>
...
// Preserve the x86 overflow flag before minizip undefines OF in ST builds.
static constexpr U32 WASM_OVERFLOW_FLAG = OF;
...
// A release while paused unlocks exactly one browser compilation probe. The first standalone, grouped, or tiny attempt consumes this token.
...
#define WASM_JIT_MT_CHAIN_BLOCK_LIMIT 32
#endif
...
// WASM i32 pointer arguments arrive signed in EM_JS. Typed-array byte offsets
// must be unsigned even when Emscripten rewrites HEAP indexing for >2 GiB heaps.
      ...
      (const void* bytes, int size, const void** importFns, int importCount),
{
    ...
    try {
#ifdef __TEST
        Module.wasmJitStandaloneModuleAttemptCount = (Module.wasmJitStandaloneModuleAttemptCount || 0) + 1;
        if (Module.wasmJitTestForceNextModuleOom) {
            Module.wasmJitTestForceNextModuleOom = false;
            throw new Error('out of memory');
        }
#endif
        var wasmBytes = new Uint8Array(HEAPU8.buffer, bytes >>> 0, size);
        ...
        var helpers = {};
        var view = new Int32Array(HEAP32.buffer, importFns >>> 0, importCount);
...
EM_JS(void, boxedwine_wasm_log_runtime_stats, (U64 compiledBlocks, U64 browserModules, U64 translatedFileBacked, U64 translatedAnonymous, U64 standaloneModules, U64 groupedModules, U64 groupedFunctions, U64 rawInputBytes, U64 mergedBytes, U64 countFlushes, U64 byteFlushes, U64 urgentFlushes, U64 processCapFlushes, U64 cancelledEntries, U64 permanentFailures, U64 tinyDeferred, U64 tinyPromoted, U64 oomPauses, U64 blockedAttempts, U64 oomRetries, U64 oomResumptions, U64 functionsPerGroup, U64 functionsPerGroupTenth), {
...
EM_JS(int, boxedwine_wasm_instantiate_runtime_batch, (const void* bytes, int size, const void** importFns, int importCount, int entryCount, int* outputSlots), {
    ...
    if (Module.wasmJitOomGeneration === Module.wasmJitReleaseGeneration) {
        return 0;
    }
    var allocated = [];
    var groupId;
    try {
...
#endif
        var module = new WebAssembly.Module(new Uint8Array(HEAPU8.buffer, bytes >>> 0, size));
        var helpers = {};
        var imports = new Int32Array(HEAP32.buffer, importFns >>> 0, importCount);
        for (var i = 0; i < importCount; ++i) {
            helpers['fn_' + i] = wasmTable.get(imports[i]);
        }
        ...
        for (var i = 0; i < entryCount; ++i) {
            ...
            if (!fn) {
                throw new Error('missing runtime batch export b' + i);
            }
        ...
        if (!Module.wasmJitRuntimeGroups) {
       ...
       U32 moduleId, U32 memoryId, U32 memoryIncarnation, S32 brokerEnabled, S32* lookupResultPtr),
{
    ...
    try {
        ...
        var nextSlotTA = new Int32Array(HEAPU8.buffer, nextSlotPtr >>> 0, 1);
...
EM_JS(void, boxedwine_wasm_release_broker_memory_mt,
    ...
    var ids = count ? Array.from(new Uint32Array(HEAPU8.buffer, moduleIds >>> 0, count)) : [];
    var slots = slotCount ? Array.from(new Int32Array(HEAPU8.buffer, tableSlots >>> 0, slotCount)) : [];
...
EM_JS(void, boxedwine_wasm_release_broker_batches_mt,
    ...
    var rows = new Uint32Array(HEAPU8.buffer, records >>> 0, recordCount * 10);
    for (var i = 0; i < recordCount; ++i) {
        ...
        var ids = moduleCount ? Array.from(new Uint32Array(
            HEAPU8.buffer, (moduleIds >>> 0) + moduleOffset * 4, moduleCount)) : [];
        var slots = slotCount ? Array.from(new Int32Array(
            HEAPU8.buffer, (tableSlots >>> 0) + slotOffset * 4, slotCount)) : [];
        var groupInstanceKeys = [];
        if (rows[base + 9] &&
       ...
       S32 brokerEnabled, S32* lookupResultPtr),
{
    ...
    try {
        ...
        if (!forcedFailure && !oom) {
            err('boxedwine_wasm_instantiate_runtime_group_mt failed: ' + error);
        }
        return oom ? -1 : 0;
    }
});
    ...
    if (runtimeGroupId !== undefined) {
        ...
        if (runtimeGroup) {
            ...
            if (runtimeGroup.slots.size === 0) {
                ...
                Module.wasmJitRuntimeGroupsReleased = (Module.wasmJitRuntimeGroupsReleased || 0) + 1;
            }
        }
    }
    if (Module.wasmJitReleaseGeneration === undefined) {
...
static __attribute__((noinline)) bool wasmJitPrepareSlotForWorker(int tableIndex, CPU* cpu, DecodedOp* op,
        std::vector<U8>& ready);

// Keep the already-installed case small enough to inline into block dispatch.
// Visibility belongs to this worker, not to the shared DecodedOp. MT indices
// are never reused, so a verified entry needs no invalidation or generation
// check. Byte entries avoid the packed-bit lookup on every compiled call.
static inline bool wasmJitSlotReadyForWorker(int tableIndex, CPU* cpu, DecodedOp* op) {
    static thread_local std::vector<U8> ready;
    if (tableIndex > 0 && (size_t)tableIndex < ready.size() && ready[(size_t)tableIndex]) {
        return true;
    }
    return wasmJitPrepareSlotForWorker(tableIndex, cpu, op, ready);
}
...
void OPCALL wasmStartJITOp(CPU* cpu, DecodedOp* op) {
    ...
    do {
        ...
        // Keep the outer run loop's interruption points between compiled calls.
        // The bound also limits time before normal thread bookkeeping resumes.
        if (++chainedBlocks >= WASM_JIT_CHAIN_BLOCK_LIMIT || cpu->yield ||
                cpu->thread->terminating || cpu->thread->process->terminated ||
                cpu->debugTrapActive ||
                cpu->thread->ptraceStopped.load(std::memory_order_acquire) ||
                cpu->jitSignalPending.load(std::memory_order_acquire) ||
                cpu->thread->pendingSignals || !cpu->nextOp) {
            return;
        }
        // A compiled exit can retain a DecodedOp across invalidation. Apply
        // the same live-cache refresh as NormalCPU::run before dereferencing it.
#ifdef __TEST
        // TestEnd is a harness sentinel rather than a guest dispatch target.
        if (cpu->nextOp->inst == TestEnd) {
            return;
        }
#endif
        op = getMemData(cpu->memory)->opCache.get(cpu->getEipAddress());
        cpu->nextOp = op;
    } while (wasmJitCanChainTo(cpu, op) && op->runCount > JIT_RUN_COUNT);
#else
    U32 chainedBlocks = 0;
...
static void wasmHelper_fetchNextOp(CPU* cpu) {
    ...
    if (!cpu->thread->terminating) {
        // Fetch from the current memory's cache: exec can replace KMemoryData.
        // A miss still takes the normal decoding and instruction-fault path.
        DecodedOp* nextOp = nullptr;
        if (cpu->thread->process) {
            nextOp = getMemData(cpu->memory)->opCache.get(cpu->getEipAddress());
            if (!nextOp) {
                nextOp = cpu->getNextOp();
            }
        }
        cpu->nextOp = nextOp;
...
static void wasmJitLogRuntimeStats() {
    ...
    U64 functionsPerGroupX10 = stats.groupedModules ? stats.groupedFunctions * 10 / stats.groupedModules : 0;
    boxedwine_wasm_log_runtime_stats(stats.translatedFileBacked + stats.translatedAnonymous, stats.standaloneModules + stats.groupedModules, stats.translatedFileBacked, stats.translatedAnonymous, stats.standaloneModules, stats.groupedModules, stats.groupedFunctions, stats.rawInputBytes, stats.mergedBytes, stats.countFlushes, stats.byteFlushes, stats.urgentFlushes, stats.processCapFlushes, stats.cancelledEntries, stats.permanentFailures, stats.tinyDeferred, stats.tinyPromoted, stats.oomPauses, stats.blockedAttempts, stats.oomRetries, stats.oomResumptions, functionsPerGroupX10 / 10, functionsPerGroupX10 % 10);
}

static void wasmJitMaybeLogRuntimeStats() {
    U64 translatedBlocks = g_wasmJitRuntimeStats.translatedFileBacked + g_wasmJitRuntimeStats.translatedAnonymous;
...
static void wasmJitRecordStandaloneTranslation(U64 rawBytes, int tableIdx, bool retryProbe) {
...
static std::unique_ptr<U32[]> allocateWasmJitRelocArray(const std::vector<U32>& relocValues) {
...
static bool isNestedWasmJitEntry(const WasmJitPendingBlock& pending, DecodedOp* op) {
    return op != pending.op && ((op->flags2 & OP_FLAG2_WASM_JIT_PENDING) || (op->pfnJitCode && op->pfn == pending.process->startJITOp));
}
...
static void failRuntimeBatch(const std::vector<WasmJitBatchEntryId>& liveIds, const char* stage, const char* detail = nullptr) {
    if (detail) {
...
static void flushRuntimeBatch(const WasmJitFlushRequest& request) {
    ...
    int result = boxedwine_wasm_instantiate_runtime_batch(merged.data(), (int)merged.size(), g_wasmHelperTable, WASM_HELPER_COUNT, (int)liveIds.size(), slots.data());
...
static void attemptTinyAnonymousPromotion(WasmJitPendingBlock& pending, bool retainOnOom) {
    bool retryProbe = wasmJitBeginOomRetryProbe();
    int tableIdx = boxedwine_wasm_instantiate(pending.bytes.data(), (int)pending.bytes.size(), g_wasmHelperTable, WASM_HELPER_COUNT);
    if (tableIdx >= 0) {
        g_wasmJitCompilationPaused = false;
        wasmJitRecordOomResumption(retryProbe);
        ++g_wasmJitRuntimeStats.standaloneModules;
        ...
        if (retainOnOom) {
            g_wasmJitSealedRequests.push_back({WasmJitSealedKind::TinyAnonymous, {pending.id}});
        }
    } else {
        U64 id = pending.id;
        ++g_wasmJitRuntimeStats.permanentFailures;
...
static void retryOldestSealedRequest() {
    while (!g_wasmJitSealedRequests.empty()) {
        ...
        entries.erase(std::remove_if(entries.begin(), entries.end(), [](WasmJitBatchEntryId id) {
...
static void queueRuntimeFlushes(const std::vector<WasmJitFlushRequest>& flushes) {
    ...
    for (const WasmJitFlushRequest& request : flushes) {
        ...
        g_wasmJitSealedRequests.push_back({WasmJitSealedKind::RuntimeBatch, request.entries});
    }
    if (!processNow) {
        return;
    }
...
static void submitPendingWasmJitBlock(CPU* cpu, DecodedOp* op, U32 address, U32 blockOpCount, U32 emulatedLen, U32 mappedFileKey, WasmJitPendingKind kind, bool needsMemoryPageArrays, std::vector<U8>&& bytes, std::vector<U32>&& relocValues) {
...
static void removePendingFromSealedRequests(WasmJitBatchEntryId id) {
    for (auto sealed = g_wasmJitSealedRequests.begin(); sealed != g_wasmJitSealedRequests.end();) {
        sealed->entries.erase(std::remove(sealed->entries.begin(), sealed->entries.end(), id), sealed->entries.end());
...
void wasmJitHandlePendingHit(CPU* cpu, DecodedOp* op) {
    ...
    if (found->second->memory != cpu->memory || found->second->state != WasmJitPendingState::Open) {
        return;
    }
...
static void wasmJitCancelPendingMemory(KMemory* memory) {
    ...
    for (WasmJitBatchEntryId id : ids) {
        auto found = g_wasmJitPendingBlocks.find(id);
        DecodedOp* op = found != g_wasmJitPendingBlocks.end() ? found->second->op : nullptr;
...
bool wasmJitTestRelocAllocationTransfer() {
    ...
    return lookupPointer && publicationArray.get() == lookupPointer && publicationArray[0] == 0x11223344 && publicationArray[1] == 0x55667788 && !pending.relocArray;
}
...
U32 wasmJitTestGetMtGroupIndexForSlot(int tableIndex) {
    std::lock_guard<std::mutex> lock(g_wasmBlockBinariesMutex);
    auto it = g_wasmMtGroupSlotRefs.find(tableIndex);
    return it == g_wasmMtGroupSlotRefs.end() ? 0 : it->second.groupIdx + 1;
}
...
#ifdef BOXEDWINE_MULTI_THREADED
// Only the first call per (slot, worker) needs JS visibility checks and lazy
// installation. Keep that work and cache growth out of the dispatch fast path;
// noinline prevents LLVM from folding it back into the small readiness check.
static bool wasmJitPrepareSlotForWorker(int tableIndex, CPU* cpu, DecodedOp* op,
        std::vector<U8>& ready) {
    if (!boxedwine_wasm_slot_check(tableIndex, (int)(uintptr_t)cpu, (int)(uintptr_t)op)) {
        if (!lazyInstallWasmJitBlockForWorker(tableIndex) ||
            !boxedwine_wasm_slot_check(tableIndex, (int)(uintptr_t)cpu, (int)(uintptr_t)op)) {
    ...
    if ((size_t)tableIndex >= ready.size()) {
        ready.resize((size_t)tableIndex + 64, 0);
    }
    ready[(size_t)tableIndex] = 1;
    return true;
}
#endif
...
void JitWasmCodeGen::emitMaterializedCondition(JitConditional cond, U32 condLocal) {
#ifdef BOXEDWINE_WASM_JIT_PROFILE
    emitProfileSampledCall(HELPER_PROFILE_INLINE_COND);
#endif
    U32 flagsLocal = allocScratch();
    m_emitter.emitLocalGet(WASM_CPU_LOCAL);
    m_emitter.emitI32Load((U32)offsetof(CPU, flags));
    m_emitter.emitLocalSet(flagsLocal);
    m_emitter.emitLocalGet(flagsLocal);
    U32 mask;
    switch (cond) {
    case JitConditional::O: case JitConditional::NO: mask = WASM_OVERFLOW_FLAG; break;
    case JitConditional::B: case JitConditional::NB: mask = CF; break;
    case JitConditional::Z: case JitConditional::NZ: mask = ZF; break;
    case JitConditional::BE: case JitConditional::NBE: mask = CF | ZF; break;
    case JitConditional::S: case JitConditional::NS: mask = SF; break;
    case JitConditional::P: case JitConditional::NP: mask = PF; break;
    default:
        // Signed comparisons test SF != OF. Align OF (bit 11) with
        // SF (bit 7), preserving ZF separately for LE/NLE below.
        m_emitter.emitLocalGet(flagsLocal);
        m_emitter.emitI32Const(4);
        m_emitter.emitOp(WASM_I32_SHR_U);
        m_emitter.emitOp(WASM_I32_XOR);
        mask = SF;
        break;
    ...
    m_emitter.emitOp(WASM_I32_AND);
    if (cond == JitConditional::LE || cond == JitConditional::NLE) {
        m_emitter.emitLocalGet(flagsLocal);
        m_emitter.emitI32Const(ZF);
        m_emitter.emitOp(WASM_I32_AND);
        m_emitter.emitOp(WASM_I32_OR);
    }
    // x86 condition encodings pair each predicate with its inverse.
    if ((U32)cond & 1) {
        m_emitter.emitOp(WASM_I32_EQZ);
    } else {
        m_emitter.emitI32Const(0);
    ...
    freeScratch(flagsLocal);
}
RegPtr JitWasmCodeGen::getCondition(JitConditional cond, RegPtr res) {
    if (currentLazyFlags == FLAGS_NONE) {
        // SSE comparisons and explicit flag writes already materialized
        // EFLAGS. Evaluate the condition without a helper or GP cache flush.
        U32 condLocal = allocScratch();
        emitMaterializedCondition(cond, condLocal);
    ...
    // Both paths only read flag state and write a scratch local (or tmpReg),
    // so dirty GP locals remain valid across this runtime branch.
    auto tmp = getTmpReg();
    bool checkMaterialized = currentLazyFlags == FLAGS_NULL;
    if (checkMaterialized) {
        m_emitter.emitLocalGet(WASM_CPU_LOCAL);
        m_emitter.emitI32Load8U((U32)offsetof(CPU, lazyFlagType));
        m_emitter.emitI32Const((S32)FLAGS_NONE);
        m_emitter.emitOp(WASM_I32_EQ);
        m_emitter.emitIf();
        emitMaterializedCondition(cond, tmp->hardwareReg());
        m_emitter.emitElse();
    }
    // The per-condition helper has signature (CPU*)->void and writes tmpReg.
    // Other incoming lazy states still need their original flag formula.
    m_emitter.emitLocalGet(WASM_CPU_LOCAL);
    ...
    // SETcc may target part of a GP register; merge through mov(b8) below.
    if (res && res != tmp) {
        mov(JitWidth::b8, res, tmp);
        freeScratch(tmp->hardwareReg());
        return res;
...
void JitWasmCodeGen::commitJIT(DecodedOp* op) {
    ...
    // Names are metadata only. Keep guest address attribution out of normal
    // builds, where module size also affects runtime batching decisions.
    BString module = cpu->thread->process->getModuleName(this->startingEip);
    const char* basename = strrchr(module.c_str(), '/');
    char name[320];
    snprintf(name, sizeof(name), "x86:p%u:%s:file_%08x:pc_%08x:ops_%u",
        cpu->thread->process->id, basename ? basename + 1 : module.c_str(),
        cpu->thread->process->getModuleEip(this->startingEip),
        this->startingEip, this->blockOpCount);
    m_emitter.setFunctionName(m_emitter.numImportedFunctions(), name);
#endif
...
#ifndef __TEST
    if (wasmJitPersistenceActive()) {
        // A cache/group hit publishes immediately. Give the lookup the exact allocation that publication will register for this table slot so no JS-created cached state can retain transient vector storage.
    ...
    if (tableIdx < 0) {
        MappedFilePtr mappedFile = cpu->thread->process->getMappedFileForRange(this->startingEip, this->emulatedLen);
        // Record the original standalone module before either online policy can move its bytes or attempt browser compilation.
        if (wasmJitPersistenceActive()) {
        ...
        if (mappedFile) {
            submitPendingWasmJitBlock(cpu, op, this->startingEip, this->blockOpCount, this->emulatedLen, mappedFile->key, WasmJitPendingKind::FileBatch, m_needsWasmMemoryPageArrays, std::move(m_wasmBinary), std::move(m_relocValues));
            return;
        }
        if (this->blockOpCount < 4) {
            submitPendingWasmJitBlock(cpu, op, this->startingEip, this->blockOpCount, this->emulatedLen, 0, WasmJitPendingKind::TinyAnonymous, m_needsWasmMemoryPageArrays, std::move(m_wasmBinary), std::move(m_relocValues));
            return;
        }
        bool retryProbe = wasmJitBeginOomRetryProbe();
        tableIdx = boxedwine_wasm_instantiate(m_wasmBinary.data(), (int)m_wasmBinary.size(), g_wasmHelperTable, WASM_HELPER_COUNT);
        wasmJitRecordStandaloneTranslation(m_wasmBinary.size(), tableIdx, retryProbe);
    }
  #else
    if (g_wasmJitTestRuntimeBatchingEnabled) {
        ...
        if (mappedFileKey || this->blockOpCount < 4) {
            submitPendingWasmJitBlock(cpu, op, this->startingEip, this->blockOpCount, this->emulatedLen, (U32)mappedFileKey, mappedFileKey ? WasmJitPendingKind::FileBatch : WasmJitPendingKind::TinyAnonymous, m_needsWasmMemoryPageArrays, std::move(m_wasmBinary), std::move(m_relocValues));
            return;
        }
    }
    bool retryProbe = wasmJitBeginOomRetryProbe();
    int tableIdx = boxedwine_wasm_instantiate(m_wasmBinary.data(), (int)m_wasmBinary.size(), g_wasmHelperTable, WASM_HELPER_COUNT);
    wasmJitRecordStandaloneTranslation(m_wasmBinary.size(), tableIdx, retryProbe);
  #endif
#endif
#ifdef BOXEDWINE_WASM_JIT_PROFILE
...
namespace {
struct WasmJitPreparedCodeInvalidation {
    KMemory* memory = nullptr;
    const std::vector<DecodedOp*>* decodedOps = nullptr;
    std::vector<S32> tableSlots;
#ifdef BOXEDWINE_MULTI_THREADED
    std::unordered_map<S32, WasmJitMtRetiredSlot> retiredSlots;
    std::unique_lock<std::mutex> blockBinariesLock;
#endif
};

void discardWasmJitCodeInvalidation(void* opaque) noexcept {
    delete static_cast<WasmJitPreparedCodeInvalidation*>(opaque);
}
...
void commitWasmJitCodeInvalidation(void* opaque) noexcept {
    ...
    if (retiredAny) {
        try {
            ...
            // bookkeeping cannot be allocated, the persistent retired-slot
            // registry keeps the release pending for a later quiescent retry.
            wasmJitMtTryReapRetired(prepared->memory);
        } catch (...) {
        }
    }
#else
    ...
    if (releasedSlot) {
        ...
        try {
            // Slot retirement is already committed. Retrying a sealed compile
            // may allocate, so contain a host allocation failure and leave the
            // sealed request available for the next real release.
...
static PreparedJitCodeInvalidation wasmJitPrepareCodeInvalidation(KMemory* memory,
    ...
    for (void* entry : jitEntries) {
        if (entry) {
            prepared->tableSlots.push_back((S32)(uintptr_t)entry);
        }
    }
    ...
    for (S32 tableIndex : prepared->tableSlots) {
        ...
        auto group = g_wasmMtGroupSlotRefs.find(tableIndex);
        if (group != g_wasmMtGroupSlotRefs.end()) {
            retired.group = true;
            retired.groupIdx = group->second.groupIdx;
            retired.moduleRef = group->second.moduleRef;
        } else {
            auto standalone = g_wasmJitMtModulesBySlot.find(tableIndex);
            if (standalone == g_wasmJitMtModulesBySlot.end()) {
                continue;
            }
            retired.moduleRef = standalone->second;
        }
...
WasmJitMtRetirementStateSnapshot wasmJitTestGetMtRetirementState(CPU* cpu) {
    WasmJitMtRetirementStateSnapshot result;
    {
        std::lock_guard<std::mutex> lock(g_wasmBlockBinariesMutex);
    ...
    }
    {
        ...
        for (const WasmJitMtReleaseBatch& release : g_wasmJitMtPendingBrokerReleases) {
            ...
            mix(release.releaseGroupInstance ? 1 : 0);
            mix(release.groupKind);
            mix(release.groupIdentity);
            mix((U32)release.moduleIds.size());
            for (U32 moduleId : release.moduleIds) {
                mix(moduleId);
            }
            mix((U32)release.tableSlots.size());
            for (S32 tableSlot : release.tableSlots) {
                mix((U32)tableSlot);
            }
        }
...
static void registerWasmJitLifecycleCallbacks() {
    ...
    std::call_once(callbacksInitFlag, []() {
        ...
        callbacks.prepareCodeInvalidation = wasmJitPrepareCodeInvalidation;
...
#endif
        callbacks.aggregatePreparedCodeInvalidation = true;
